...
import RequestTabsRefactored from './api-test/RequestTabsRefactored';
import ResponseViewer from './api-test/ResponseViewer';
import HistorySelector from './api-test/HistorySelector';
...
interface MainContentFinalProps {
  selectedItem: ApiItem | null;
  onResetForm: () => void;
  onUpdateSelectedItem: (updatedItem: Partial<ApiItem>) => void;
}

const MainContentFinal: React.FC<MainContentFinalProps> = ({
  selectedItem,
  onResetForm,
  ...
  const handleSaveApi = async () => {
    if (!selectedItem) {
      showToast('warning', '저장할 아이템이 선택되지 않았습니다.');
      return;
    }
    ...
    try {
      ...
      showToast('error', `API 저장 중 오류가 발생했습니다: ${error.response?.data?.message || error.message}`);
    }
    setSaving(false);
  };
  ...
  const handleSaveHistory = async (historyName: string) => {
    ...
    try {
      ...
      showToast('success', '히스토리가 성공적으로 저장되었습니다!');
    } catch (error: any) {
      console.error('Failed to save history:', error);
      showToast('error', `히스토리 저장 중 오류가 발생했습니다: ${error.response?.data?.message || error.message}`);
    } finally {
...
export default MainContentFinal;